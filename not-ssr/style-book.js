...
domReady(() => {
  ...
  const theatrumIndex = categories.findIndex(({ slug }) => slug === 'theatrum');
  const widgetsIndex = categories.findIndex(({ slug }) => slug === 'widgets');
  ...
  const newWidgetsIndex = reordered.findIndex(({ slug }) => slug === 'widgets');
