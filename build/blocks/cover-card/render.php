...
// Get featured image, falling back to the theme's per-post-type default
// (chance_get_fallback_image) when the post has no thumbnail of its own.
$featured_image_url = '';
if (has_post_thumbnail($post->ID)) {
  $featured_image_url = get_the_post_thumbnail_url($post->ID, 'full');
} elseif (function_exists('chance_get_fallback_image')) {
  $featured_image_url = chance_get_fallback_image($post->post_type);
}
...
<div <?php echo wp_kses_data( $wrapper_attributes ); ?>>
  <div class="cover-card" style="<?php echo esc_attr($bg_style); ?>">
    <a class="cover-card__link" href="<?php echo esc_url($post_permalink); ?>" <?php echo $target; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $target is a fixed literal (' target="_blank" rel="noopener noreferrer"' or ''). ?> aria-label="<?php echo esc_attr($post_title); ?>"></a>
    <div class="user-content"><?php echo do_blocks($content); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- InnerBlocks output rendered by the block pipeline. ?></div>
