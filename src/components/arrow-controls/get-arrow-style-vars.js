 ...
 * property names its own stylesheet reads, so the two blocks stay
 * independent even though they share this same attribute shape. Leaving a
 * value `undefined` omits that custom property entirely, letting the
 * stylesheet's own `var(--x, fallback)` default apply.
 *
 ...
 */
export default function getArrowStyleVars( attributes, { prefix } ) {
	const {
		arrowBackground,
		arrowColor,
