...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	useEffect(() => {
		if (innerBlocks.length > 0) {
			setFaces((prev) => (prev.front ? prev : { front: innerBlocks[0].attributes, back: null }));
		}
	}, [innerBlocks]);
	...
	const updateFlip = (index) => {
		...
		setHoverIndex(index);
		setFaces((prev) =>
			index % 2 ? { ...prev, back: block.attributes } : { ...prev, front: block.attributes }
		);
	};
	...
	const handleMouseOver = (event) => {
		...
		const index = innerBlocks.findIndex((block) => block.clientId === itemEl.dataset.block);
		if (index !== -1) {
			updateFlip(index);
		}
	};
			...
			<div {...blockProps}>
				<div className="thumbnail-list-wrapper">
					...
					<div
						...
						style={{ order: thumbnailPosition === 'left' ? -1 : 0 }}
					>
						<div
							className="thumbnail-flipper"
							style={{ transform: `rotateX(${hoverIndex * -180}deg)` }}
						>
							<img
