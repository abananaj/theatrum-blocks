 ...
 *
 * @param {Object}      props
 * @param {Array}       props.tabs              Raw theatrum/tab block objects.
 * @param {Array}       props.tabPanels         Raw theatrum/tab-panel block objects.
 * @param {string|null} props.tabPanelsClientId Client ID of the theatrum/tab-panels block.
 * @param {string|null} props.tabsListClientId  Client ID of the theatrum/tab-list block.
 */
		...
		if (tabPanelCountChange < 0) {
			...
			const newTabs = tabPanels
				.map((tabPanel, tabPanelIndex) =>
					!prevTabPanelIds.has(tabPanel.clientId)
						? {
							tabPanelIndex,
							block: createBlock('theatrum/tab', {}),
						}
						: null
				)
				.filter(Boolean);
			...
			const newTabPanels = tabs
				.map((tab, tabIndex) => {
					...
					return {
						tabIndex,
						block: createBlock('theatrum/tab-panel', { label }),
					};
				})
				.filter(Boolean);
