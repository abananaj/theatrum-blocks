...
function block_core_tabs_generate_tabs_list(array $innerblocks = array(), string $tabs_id = ''): array
{
	...
	foreach ($innerblocks as $inner_block) {
		if ('theatrum/tab-panels' === ($inner_block['blockName'] ?? '')) {
			$tab_index = 0;
			foreach ($inner_block['innerBlocks'] ?? array() as $tab_block) {
				if ('theatrum/tab-panel' === ($tab_block['blockName'] ?? '')) {
...
/**
 * Filter to provide tabs list context to theatrum/tabs and theatrum/tab-list blocks.
 * It is more performant to do this here, once, rather than in the tabs render and tabs context filters.
 * In this way theatrum/tabs is both a provider and a consumer of the theatrum/tabs-list context.
 *
 * @since 7.0.0
 *
...
{
	if ('theatrum/tabs' === $parsed_block['blockName']) {
		...
		$context['theatrum/tabs-list'] = $tabs_list;
		$context['theatrum/tabs-id']   = $tabs_id;
	}
...
/**
 * Render callback for theatrum/tabs.
 *
 * @since 7.0.0
 *
...
function block_core_tabs_render_block_callback(array $attributes, string $content, \WP_Block $block): string
{
	...
	$tabs_list        = $block->context['theatrum/tabs-list'] ?? array();
	$tabs_id          = $block->context['theatrum/tabs-id'] ?? null;
	...
	$tag_processor->set_attribute('data-wp-interactive', 'theatrum/tabs/private');

	// Inspect inside the tab-list to see if its vertical or not.
	$tag_processor->set_bookmark('theatrum/tabs_wrapper');
	...
	$tag_processor->seek('theatrum/tabs_wrapper');
	 ...
	 * client side state management per theatrum/tabs instance, like context.
	 */
	wp_interactivity_state(
		'theatrum/tabs/private',
		array(
			$tabs_id => $tabs_list,
		)
	);
...
/**
 * Registers the `theatrum/tabs` block on the server.
 *
 * @since 7.0.0
 */
