			...
			<div style={{ marginTop: '8px' }}>
				...
				{displayItems.map((item, index) => (
					<p key={index} style={{ margin: '4px 0 0 0' }}>
						...
						{item.meta_title && (
							<span className="site-option-meta"> {renderAffix(item.meta_title, attributes.appendTag)}</span>
						)}
					</p>
				))}
				...
				<div style={{ padding: '16px' }}>
					...
					{!isMemberType && (
						<ToggleControl
							...
							onChange={(value) => setAttributes({ linkPostTitle: value })}
					...
					<SelectControl
						...
						onChange={(value) => setAttributes({ prependTag: value })}
						__nextHasNoMarginBottom
						__next40pxDefaultSize
					/>
					<TextControl
						...
						help={attributes.metaKey ? 'Disabled while a Post Meta Key is set. Use Append Style to style the meta value instead.' : undefined}
						__nextHasNoMarginBottom
						__next40pxDefaultSize
					/>
					<SelectControl
						...
						options={AFFIX_STYLE_OPTIONS}
						onChange={(value) => setAttributes({ appendTag: value })}
						help={attributes.metaKey ? 'Styles the post meta value.' : undefined}
						__nextHasNoMarginBottom
						__next40pxDefaultSize
					/>
				</div>
