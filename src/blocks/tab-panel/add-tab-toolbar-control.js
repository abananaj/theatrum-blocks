 ...
 * Inserts a new theatrum/tab-panel into the tab-panels and a new theatrum/tab
...
export default function AddTabToolbarControl({ tabsClientId }) {
	...
	const { tabPanelsClientId, tabsListClientId } = useSelect(
		(select) => {
			...
			const tabPanels = innerBlocks.find(
				(block) => block.name === 'theatrum/tab-panels'
			);
			const tabList = innerBlocks.find(
				(block) => block.name === 'theatrum/tab-list'
			);
			return {
	...
	const addTab = () => {
		...
		const newTabPanelBlock = createBlock('theatrum/tab-panel', {
			label: __('Tab'),
		});
		...
		if (tabsListClientId) {
			const newTabBlock = createBlock('theatrum/tab', {});
