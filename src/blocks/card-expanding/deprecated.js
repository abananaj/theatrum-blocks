 ...
 * content flow can't be given a width, aspect ratio or object fit of its own without fighting
...
const IMAGE_DEFAULTS = {
	...
	imageAspectRatio: metadata.attributes.imageAspectRatio.default,
...
const v3 = {
	...
	save( { attributes } ) {
		...
		const blockProps = useBlockProps.save( {
			style: getCardImageVars( attributes ),
		} );
		const innerBlocksProps = useInnerBlocksProps.save( {
			className: 'wp-block-theatrum-card-expanding__content',
		} );
...
const v1 = {
	...
	migrate( attributes ) {
		...
		// The old title was a <span> inside the trigger; a heading is what view.js now looks for.
		innerBlocks.push(
