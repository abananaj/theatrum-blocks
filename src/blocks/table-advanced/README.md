...
The editor view uses `<div>` elements styled to look like a table (via the `.editor-styles-wrapper .tm-edit-*` rules in `style.scss`) rather than actual table elements. This avoids browser HTML parsing rules that would eject Gutenberg's UI chrome (`<div>` wrappers) out of a real `<table>`. The frontend `save()` output is correct semantic HTML.
...
- **Block appender (+ button) in rows** — The "Add block" button that appears at the end of a row's inner blocks is styled as `flex: 0 0 auto` to prevent it from taking up a full column slot, but it may still look visually out of place. Refine `.block-list-appender` targeting in `style.scss` if needed.
