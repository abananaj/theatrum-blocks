...
/**
 * Onstage Next block - file template
...
if ($production_id > 0) {
  ...
  // Use automatic next production
  if (!function_exists('chance_get_next_production')) {
    return;
  }
  $prod_data = chance_get_next_production();
}

if (!$prod_data) {
  echo '<p>' . esc_html__('No upcoming production scheduled.', 'onstage-next') . '</p>';
  return;
}
