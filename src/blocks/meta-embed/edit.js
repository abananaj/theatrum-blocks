...
function getAspectRatioClassName(width, height, allowResponsive) {
  if (!allowResponsive || !width || !height) return '';
  const ratio = width / height;
  const closest = ASPECT_RATIOS.reduce((best, candidate) => {
    if (!best) return candidate;
    return Math.abs(candidate.ratio - ratio) < Math.abs(best.ratio - ratio) ? candidate : best;
  }, null);
...
export default function Edit({ attributes, setAttributes, context }) {
  ...
  const hasEmbed = (isYouTube && youTubeId) || (!isYouTube && embedData?.html);
  ...
  if (isYouTube && youTubeId) {
    ...
    aspectClassName = getAspectRatioClassName(width || 16, height || 9, allowResponsive);
  }

  const blockProps = useBlockProps({ className: aspectClassName || undefined });

  const toggleResponsive = () => setAttributes({ allowResponsive: !allowResponsive });
      ...
      <InspectorControls>
        <PanelBody title={__('Settings', 'theatrum-blocks')}>
          ...
          {hasEmbed && (
            <ToggleControl
              __nextHasNoMarginBottom
              label={__('Resize for smaller devices', 'theatrum-blocks')}
              checked={allowResponsive}
              help={
                allowResponsive
                  ? __('This embed will preserve its aspect ratio when the browser is resized.', 'theatrum-blocks')
                  : __('This embed may not preserve its aspect ratio when the browser is resized.', 'theatrum-blocks')
              }
