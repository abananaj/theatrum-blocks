...
/**
 * Render callback for theatrum/tab-list.
 *
...
function block_core_tab_list_render_callback(array $attributes, string $content, \WP_Block $block): string
{
	...
	foreach ($block->parsed_block['innerBlocks'] ?? array() as $parsed_tab) {
		if ('theatrum/tab' !== ($parsed_tab['blockName'] ?? '')) {
			continue;
		}
		...
		$item_context = array_merge(
			$block->context,
			array(
				'theatrum/tab-index' => $tab_index,
				'theatrum/tab-id'    => $tab['id'] ?? '',
				'theatrum/tab-label' => $tab['label'] ?? '',
			)
		);
...
/**
 * Registers the `theatrum/tab-list` block on the server.
 *
 * @since 7.0.0
 */
