...
export default function Edit({ attributes, setAttributes }) {
  ...
  return (
    <>
      <InspectorControls>
        <PanelBody title={__('Production Selection', 'onstage-current')}>
          <SearchControl
          ...
          {!productionId && (
            <p className="help-text">
              {__(
                'Leave empty to show the automatically determined current or next production.',
                'onstage-current'
              )}
            </p>
          )}
          {productionId && selectedPost && (
            <div className="selected-production">
              <p>
                <strong>{__('Selected:', 'onstage-current')}</strong>
              </p>
              ...
              <button
                ...
                className="button"
              >
                {__('Clear Selection', 'onstage-current')}
              </button>
            </div>
          )}
        </PanelBody>

        <PanelBody title={__('Button Settings', 'onstage-current')}>
          <TextControl
            label={__('Button Text', 'onstage-current')}
            value={buttonText}
            onChange={(value) =>
      ...
      <div {...useBlockProps()}>
        <div className="wp-block-cover wp-block-cover__inner-container">
          ...
          <div className="wp-block-cover__content">
            {selectedPost ? (
              <p>
                {__('Selected Production:', 'onstage-current')}{' '}
                <strong>{selectedPost.title.rendered}</strong>
              </p>
            ) : (
              <p>
                {__(
                  'Current or Next Production (automatic)',
                  'onstage-current'
                )}
              </p>
            )}
