 ...
 *
 * The panel only appears inside an accordion carrying `is-style-ct-accordion-icon`, which is why it
...
const STYLE_CLASS = 'is-style-ct-accordion-icon';
...
function buildCardProps( attributes ) {
	...
	if ( attributes?.ctIconUrl ) {
		...
		style[
			'--ct-accordion-icon-glyph'
		] = `url('${ attributes.ctIconUrl }')`;
		classNames.push( ICON_CLASS );
	}

	if ( attributes?.ctIconBackground ) {
		style[ '--ct-accordion-icon-rail-bg' ] = attributes.ctIconBackground;
	}
		...
		<InspectorControls>
			...
			>
				...
				>
					...
					>
						<div className="ct-accordion-icon-media">
							{ ctIconUrl && (
								<span
									aria-hidden="true"
									className="ct-accordion-icon-preview"
									style={ {
										'--ct-accordion-icon-glyph': `url('${ ctIconUrl }')`,
									} }
								/>
							) }
							<MediaUploadCheck>
				...
				>
					<BaseControl
						__nextHasNoMarginBottom
						id="ct-accordion-icon-background"
						label={ __( 'Icon background', 'theatrum-blocks' ) }
						help={ __(
