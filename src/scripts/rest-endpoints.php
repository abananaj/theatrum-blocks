...
function register_production_credits_rest_endpoint()
{
	register_rest_route('chance/v1', '/production-credits/(?P<post_id>\d+)', array(
		'methods'             => 'GET',
		'callback'            => 'theatrum_get_production_credits_rest_callback',
		'permission_callback' => 'theatrum_editor_permission_check',
	));
}

add_action('rest_api_init', 'register_production_credits_rest_endpoint');

function theatrum_get_production_credits_rest_callback($request)
{
	$post_id = intval($request['post_id']);
